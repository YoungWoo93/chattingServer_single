...
public:
	...
	/// </returns>
	inline UINT32 cancelIOUnregist();
	...
	/// false : 아님
	/// </returns>
	inline bool hasCancelIOFlag();

	SOCKET getSocket();
	UINT16 getPort();
	ULONG getIp();

	inline bool sendOverlappedCheck(OVERLAPPED* o) {
		return o == &sendOverlapped;
	}
	inline bool recvOverlappedCheck(OVERLAPPED* o) {
...
public:
	...
	SOCKET socket;						//8
	UINT16 port;						//2
	volatile short sendFlag;			//2		- interlock
	ULONG ip;							//4
	...
	volatile UINT32 IOcount;			//4		- interlock
	short cancelIOFlag;					//2
	LockFreeQueue<serializer*> sendBuffer;			//72(8)
